...
(function() {
	...
	var pathA = document.getElementById('pathA'),
		...
		toCloseIcon = true,
		wrapper = document.getElementById('showMenu');
	...
	function init() {
		var showMenu = document.getElementById( 'showMenu' ),
			...
			containermenu = perspectiveWrapper.querySelector( '.containermenu' ),
			contentWrapper = containermenu.querySelector( '.wrapper' );
      ...
      $(".menu-icon-wrapper svg path").css(
		...
		containermenu.addEventListener( clickevent, function( ev ) {
			if( classie.has( perspectiveWrapper, 'animate') ) {
				var onEndTransFn = function( ev ) {
					if( support && ( ev.target.className !== 'containermenu' || ev.propertyName.indexOf( 'transform' ) == -1 ) ) return;
					outAC(segmentA);
